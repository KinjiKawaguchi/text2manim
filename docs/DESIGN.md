...
Cloud Run 運用の前提: エフェメラルファイルシステムのため、SQLite とローカル動画保存は
インスタンス再起動で消える。永続運用にはストレージ抽象 (GCS / S3互換) が必要で、
これをコンテナイメージ配布とセットのマイルストーンとする。
...
- **ストレージ抽象 (GCS / S3互換)**: エフェメラルな実行環境での永続化。Cloud Run運用の前提
